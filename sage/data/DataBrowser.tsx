...
function LineChart(props) {
  ...
  const grouped = groupBy(data, 'sensor')

  const datasets = Object.keys(grouped)
    .map((sensor, i) => {
      const d = grouped[sensor].map(o => ({
        ...
        <Main>
          {lastN &&
            <Alert severity="info">
              ...
              Listing last {lastN.limit.toLocaleString()}.
            </Alert>
          }
          ...
          {data && data?.length > 0 &&
            <Table
              ...
              pagination
              page={page}
              rowsPerPage={(app || '').match(/image|audio/g) ? 20 : 100}
              ...
              middleComponent={
                <RangeCtrls className="flex items-center">
                  {data &&
